class SellersController < ApplicationController
  ...
  def show
    ...
    # @installments_on_date = @seller.operations.includes(invoices: :installments).where(invoices: {installments: {"paid = :paid AND due_date < :time", {paid: true, time: Time.now}}}).group_by_month(:deposit_date, format: "%b %y").sum("value_cents")
    # @installments_on_date = @seller.operations.includes(invoices: :installments).where("invoice.installment.paid = :paid AND invoice.installment.due_date < :time", {paid: true, time: Time.now}).group_by_month(:deposit_date, format: "%b %y").sum("value_cents")
    # @installments_on_date = @seller.operations.includes(invoices: :installments).where("operations.invoices.installments.paid = :paid AND operations.invoices.installments.due_date < :time", {paid: true, time: Time.now}).group_by_month(:deposit_date, format: "%b %y").sum("value_cents")
